...
shell> cat /etc/passwd                      # 查看所有用户账号，格式 = 用户名:口令:用户标识号:组标识号:注释性描述:主目录:登录 Shell
                                            # 用户标识号：0 是超级用户 root 的标识号，1~99 由系统保留
...
shell> useradd -G guest,stranger kasei          # 添加用户的同时，指定用户所属的 附加用户组 为 guest 和 stranger
...
shell> usermod -c -d -m -g -G -s -u -o          # 跟 useradd 中一样的
